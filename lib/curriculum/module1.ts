export const curriculumModule1 = [
  {
    ...
    description: "Welcome to web styling! 🖼️ Let's learn how to give simple shapes (boxes) size, color, and spacing. Think of it like digital drawing and coloring, no experience needed!",
    lessons: [
      {
        ...
        focus_concept: "Understanding 'width' as a dimension and how Tailwind's w-{number} controls it. Introduction to Tailwind's unit system.",
        instruction: "Web pages use **pixels (px)** for size. Let's make this box **128 pixels wide**. In Tailwind, add the class `w-32` to do this. (The gray background and initial height are already set up for you 👍).",
        starter_component_jsx: "<div class=\"h-16 bg-slate-200\"></div>",
        target_classes: ["w-32"],
        explanation: {
          intro: "Imagine your screen is a grid of tiny lights 💡 – these are pixels! '128 pixels wide' means it spans 128 of these dots horizontally.",
          class_details: [
            {
              className: "w-32",
              css_equivalent: "width: 8rem; /* 128px */",
              description: "Makes the box 128px wide. Tailwind's magic rule for these numbers: **Tailwind Number * 4 = Pixels (px)**. So, `w-32` is `32 * 4px = 128px`."
            }
          ],
          key_takeaway: "`w-{number}` controls width. Pixel width = Tailwind Number * 4.",
          expert_tip: "Exact pixel sizes are a start, but soon we'll explore responsive sizes that adapt to different screens! 📱💻"
        },
      ...
      },
      {
        ...
        instruction: "Our box is already 128px wide (`w-32`) and gray. Now, let's make it **128 pixels tall** by adding `h-32`.",
          ...
          class_details: [
            {
              ...
              description: "Makes the box 128px tall. Same rule: **Tailwind Number * 4 = Pixels (px)**. So, `h-32` means `32 * 4px = 128px`."
            }
          ],
          key_takeaway: "`h-{number}` controls height, using the same 'Multiply by 4' rule.",
          expert_tip: "Sometimes height is set by content (`h-auto`) or fills available space (`h-full`). More on these later!"
        },
          ...
          class_details: [
            {
              ...
              description: "Sets a blue background. Tailwind colors have shade numbers like `100` (light) to `900` (dark). `500` is a nice, balanced mid-tone."
            }
          ],
          key_takeaway: "`bg-{colorName}-{shadeNumber}` sets background color. Lower shades = lighter, higher shades = darker.",
      ...
      },
      {
        ...
        instruction: "Our blue box (`w-32 h-32 bg-blue-500`) has text. It's a bit cramped! **Padding** adds 'cushioning' *inside*. Add 16 pixels of padding on all sides with `p-4`.",
        starter_component_jsx: "<div class=\"w-32 h-32 bg-blue-500 text-white flex items-center justify-center text-sm font-medium\">TEXT</div>",
        target_classes: ["p-4"],
        explanation: {
          intro: "Padding is like the empty space inside a photo frame, around the photo. 🖼️ It gives content 'breathing room'.",
          class_details: [
            {
              className: "p-4",
              css_equivalent: "padding: 1rem; /* 16px */",
              description: "Adds 16px padding to all four sides (top, right, bottom, left). Rule: **Tailwind Number * 4 = Pixels (px)**, so `p-4` means `4 * 4px = 16px`."
            }
          ],
      ...
      },
      {
        ...
        instruction: "Our blue box has uniform `p-4`. Let's change it: 32px padding horizontally (add `px-8`), and 16px vertically (add `py-4`). You'll need to *remove* the old `p-4` class first!",
        starter_component_jsx: "<div class=\"w-32 h-32 bg-blue-500 text-white flex items-center justify-center text-sm font-medium p-4\">TEXT</div>",
        target_classes_to_remove: ["p-4"],
        target_classes: ["px-8", "py-4"],
        explanation: {
          intro: "Often, you want different padding for horizontal (↔️ left/right) and vertical (↕️ top/bottom) directions.",
          class_details: [
            {
              className: "px-8",
      ...
      },
      {
        ...
        instruction: "Our blue box has `px-8 py-4`. For max control, let's set padding for each side: 8px top (`pt-2`), 16px right (`pr-4`), 24px bottom (`pb-6`), and 32px left (`pl-8`). Remove `px-8` and `py-4` first.",
        starter_component_jsx: "<div class=\"w-32 h-32 bg-blue-500 text-white flex items-center justify-center text-xs font-medium px-8 py-4\">TEXT</div>",
        ...
        explanation: {
          ...
          expert_tip: "While powerful, use symmetrical padding (`p-`, `px-`, `py-`) when possible for consistency. These are for unique cases."
        },
      ...
      },
      {
        ...
        instruction: "See the blue box and the gray box? They're touching! **Margin** is space *outside* a box. On the blue box (`w-24 h-24 bg-blue-500`), add a 16px margin on all sides with `m-4`. This will push it away.",
        starter_component_jsx: "<div class=\"flex items-center p-1 bg-slate-100\"><div class=\"w-24 h-24 bg-slate-300\"></div><div class=\"w-24 h-24 bg-blue-500\"></div></div>",
        ...
        explanation: {
          intro: "Margin is like an invisible force field 🛡️ around an element, pushing other elements away. It's space *outside* its border.",
          class_details: [
            {
              className: "m-4",
          ...
          expert_tip: "CSS has 'margin collapsing' for vertical margins, which can be tricky. Flexbox/Grid `gap` utilities often give more predictable spacing between items."
        },
      ...
      },
      {
        ...
        instruction: "On the blue box (`w-24 h-24 bg-blue-500`), let's set axis margins: 32px horizontally (add `mx-8`), and 8px vertically (add `my-2`).",
        starter_component_jsx: "<div class=\"flex flex-col items-center p-1 bg-slate-100\"><div class=\"w-full h-4 bg-slate-300\"></div><div class=\"flex items-center\"><div class=\"w-4 h-24 bg-slate-300\"></div><div class=\"w-24 h-24 bg-blue-500\"></div><div class=\"w-4 h-24 bg-slate-300\"></div></div><div class=\"w-full h-4 bg-slate-300\"></div></div>",
        ...
        explanation: {
          ...
          key_takeaway: "`mx-{number}` for horizontal, `my-{number}` for vertical margins. `mx-auto` is special for centering block items horizontally!",
          expert_tip: "`mx-auto` on a block element with a defined width will center it horizontally within its parent."
        },
      ...
      },
      {
        ...
        instruction: "Take a box (`w-32 h-32 bg-slate-100`). Give it a 2px wide solid red border. Add `border-2` for width and `border-red-500` for color.",
        ...
        explanation: {
          ...
          key_takeaway: "For a visible border, you need width (e.g., `border`, `border-2`) AND color (e.g., `border-slate-500`).",
          expert_tip: "You can add borders to individual sides: `border-t-2` (top), `border-b-4` (bottom), etc. Great for separators!"
        },
      ...
      },
      {
        ...
        instruction: "Our box now has a red border (`w-32 h-32 bg-slate-100 border-2 border-red-500`). Let's make its corners nicely rounded. Add `rounded-lg`.",
          ...
          class_details: [
            {
              ...
              description: "Applies a 'large' border radius. Sizes: `rounded-sm` (small), `rounded` (default), `rounded-md` (medium), `rounded-lg` (large), up to `rounded-3xl`, and `rounded-full` (circle/oval)."
            }
          ],
      ...
      },
      {
        ...
        instruction: "Let's build a basic 'card' shell by combining what we've learned! Add classes to make it: 256px wide (`w-64`), white background (`bg-white`), 16px all-sides padding (`p-4`), a light gray 1px border (`border border-slate-200`), medium rounded corners (`rounded-md`), and a subtle shadow (`shadow-md` - bonus!).",
        ...
        explanation: {
          ...
          class_details: [
            { className: "shadow-md", description: "(Bonus! 🎉) Adds a subtle `box-shadow` for a 'lifted' look. Shadows add depth!" }
          ],
          key_takeaway: "Real UIs are built by combining these fundamental properties. This card shell is a common pattern.",
          expert_tip: "Think in layers: base (size/bg), inner space (padding), edge (border/radius), effects (shadows). Consistency is key! 🔑"
        },
