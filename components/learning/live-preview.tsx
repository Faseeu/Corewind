...
export function LivePreview({
  ...
  useEffect(() => {
    console.log("LivePreview effect running with:", {
      appliedClasses,
      starter_component_jsx,
      starter_html_structure,
      target_classes_applied_to_selector,
    ...
    if (!hostElement || !hostElement.children[0]) {
      console.warn("No content host element or children found")
    ...
    console.log("Rendered output element:", renderedOutputElement)
    ...
    if (starter_component_jsx) {
      ...
      if (target_classes_applied_to_selector) {
        ...
        if (selected instanceof HTMLElement) {
          ...
          console.log("Found target element with selector:", target_classes_applied_to_selector)
        } else {
          console.warn(
      ...
      if (targetElement) {
        console.log("Applying classes to target element:", fullClassName)
    ...
    // Handle legacy format with starter_html_structure
      ...
      console.log("Applying classes to wrapper element:", fullClassName)
  ...
  const renderComponent = () => {
    ...
    if (starter_component_jsx) {
      console.log("Using starter_component_jsx:", starter_component_jsx)
    ...
    if (starter_html_structure) {
      console.log("Using starter_html_structure:", starter_html_structure)
    ...
    const newClassName = appliedClasses.join(" ")
    const fullClassName = `${baseTransitionClasses} ${newClassName}`.trim()
    console.log("Using fallback component with classes:", fullClassName)
      ...
      <div className="flex-1 p-8 overflow-auto">
        <div className="h-full flex items-center justify-center">
          ...
          >
            ...
            <div ref={contentHostRef} className="w-full h-full flex items-center justify-center">
              {renderComponent()}
            </div>
          </div>
        </div>
